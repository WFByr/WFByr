...
- (void)configNoDataDefaultViewWithViewType:(NoDataDefaultViewType)viewType isHasData:(BOOL)isHasData handle:(HandleBlock)handle {
    ...
    if (viewType == NoDataDefaultViewTypeNoData) {
        [self configNoDataDefaultViewWithViewType:NoDataDefaultViewTypeNoData imageName:@"defaultView_NoData" title:@"没有更多数据了" detail:nil isHasData:isHasData handle:nil];
    }
    else {
        [self configNoDataDefaultViewWithViewType:NoDataDefaultViewTypeNoData imageName:@"defaultView_NoNetwork" title:@"网络竟然崩溃了" detail:@"试试看刷新页面~" isHasData:isHasData handle:handle];
    }
}
